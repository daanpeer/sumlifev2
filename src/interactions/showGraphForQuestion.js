...
const groupAvgAnswersByWeek = async (answers, question) =>
  Object.entries(groupQuestionsByWeek(answers))
    .reduce((acc, [weekNumber, questions], index) => {
...
const showGraphForQuestion = async (ctx) => {
  ...
  try {
    const response = await fetch(`${GRAPH_URL}`, {
      method: 'POST',
      body: JSON.stringify({
        filename: `${crypto.createHash('md5').update(JSON.stringify(data)).digest('hex')}.png`,
        chartData: data
      })
    })
